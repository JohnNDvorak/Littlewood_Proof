...
KEY RESULTS:
- perron_horizontal_bound_pointwise: |y^{x+iT}/(x+iT)| ≤ y^x/T
- integral_norm_bound_large_y: ∫ ‖y^z/z‖ ≤ (y^c - y^{-R})/(T log y) for y > 1
- integral_boundary_rect_perron_pos: ∮ y^z/z = 2πi for y > 1
- integral_boundary_rect_perron_neg: ∮ y^z/z = 0 for y < 1
-/
...
lemma perron_horizontal_bound_pointwise (y : ℝ) (hy : 0 < y) (x : ℝ) (T : ℝ) (hT : 0 < T) :
    ‖(y : ℂ)^(x + I * T) / (x + I * T)‖ ≤ y^x / T := by
      norm_num [ Complex.norm_exp, Complex.norm_cpow_eq_rpow_re_of_pos hy ];
      field_simp;
      exact div_le_one_of_le₀ ( Real.le_sqrt_of_sq_le ( by norm_num [ Complex.normSq ] ; nlinarith ) ) ( Real.sqrt_nonneg _ )
...
lemma integral_y_pow_div_T (y : ℝ) (hy : 0 < y) (hy1 : y ≠ 1) (T : ℝ) (hT : T ≠ 0) (a b : ℝ) :
    ∫ x in a..b, y^x / T = (y^b - y^a) / (T * Real.log y) := by
...
lemma integral_norm_bound_large_y (y : ℝ) (hy : 1 < y) (c : ℝ) (T : ℝ) (hT : 0 < T) (R : ℝ) (hR : -R ≤ c) :
    ∫ x in -R..c, ‖(y : ℂ)^(x + I * T) / (x + I * T)‖ ≤ (y^c - y^(-R)) / (T * Real.log y) := by
      -- Let's choose any $x \in [-R, c]$.
      have h_integral_bound : ∫ x in -R..c, ‖(y : ℂ) ^ (x + Complex.I * T) / (x + Complex.I * T)‖ ≤ ∫ x in -R..c, y^x / T := by
...
lemma integral_norm_bound_small_y (y : ℝ) (hy : 0 < y) (hy1 : y < 1) (c : ℝ) (T : ℝ) (hT : 0 < T) (R : ℝ) (hR : c ≤ R) :
    ∫ x in c..R, ‖(y : ℂ)^(x + I * T) / (x + I * T)‖ ≤ (y^c - y^R) / (T * |Real.log y|) := by
      -- Use `intervalIntegral.integral_mono_on` to bound the integral of the norm by the integral of `fun x => y^x / T`.
      have h_bound : ∫ x in c..R, ‖(y : ℂ)^(x + Complex.I * T) / (x + Complex.I * T)‖ ≤ ∫ x in c..R, y^x / T := by
        apply_rules [ intervalIntegral.integral_mono_on ];
        · apply_rules [ ContinuousOn.intervalIntegrable ];
...
lemma vertical_integral_bound_far_left (y : ℝ) (hy : 0 < y) (R : ℝ) (hR : 0 < R) (T : ℝ) (hT : 0 < T) :
    ‖∫ t in -T..T, (y : ℂ)^(-R + I * t) / (-R + I * t)‖ ≤ 2 * T * y^(-R) / R := by
      -- The absolute value of the integrand is \( \left| \frac{y^{-R + it}}{-R + it} \right| = \frac{y^{-R}}{|-R + it|} \leq \frac{y^{-R}}{R} \).
      have h_abs_left : ∀ t ∈ Set.Icc (-T : ℝ) T, ‖(y : ℂ)^(-R + Complex.I * t) / (-R + Complex.I * t)‖ ≤ y^(-R : ℝ) / R := by
...
lemma vertical_integral_bound_far_right (y : ℝ) (hy : 0 < y) (R : ℝ) (hR : 0 < R) (T : ℝ) (hT : 0 < T) :
    ‖∫ t in -T..T, (y : ℂ)^(R + I * t) / (R + I * t)‖ ≤ 2 * T * y^R / R := by
      ...
      have h_integrand_norm : ∀ t : ℝ, ‖(y : ℂ)^(R + Complex.I * t) / (R + Complex.I * t)‖ = y^R / Real.sqrt (R^2 + t^2) := by
        ...
        norm_num [ ← mul_add, Real.exp_mul, Real.exp_log hy ];
        rw [ Real.sqrt_sq ( by positivity ), ← Real.exp_mul, mul_comm, Real.exp_mul, Real.exp_log hy ];
      -- Since $|R + it| \geq R$, we have $\frac{y^R}{|R + it|} \leq \frac{y^R}{R}$.
      have h_integrand_bound : ∀ t : ℝ, y^R / Real.sqrt (R^2 + t^2) ≤ y^R / R := by
      ...
      exacts [ y ^ R / R, fun t ht => h_integrand_norm t ▸ h_integrand_bound t, by ring_nf; norm_num [ abs_of_pos, hT ] ; nlinarith [ Real.rpow_pos_of_pos hy R, mul_inv_cancel₀ ( ne_of_gt hR ) ] ]
...
lemma integral_norm_bound_large_y_improper (y : ℝ) (hy : 1 < y) (c : ℝ) (T : ℝ) (hT : 0 < T) :
    ∫ x in Iic c, ‖(y : ℂ)^(x + I * T) / (x + I * T)‖ ≤ y^c / (T * Real.log y) := by
      -- We use the fact that $\int_{-\infty}^{c} y^x / T \, dx = \frac{y^c}{T \log(y)}$.
      have h_integral : ∫ x in Set.Iic c, (y : ℝ) ^ x / T = (y ^ c) / (T * Real.log y) := by
        have h_integrable_large_y : ∫ x in Set.Iic c, y^x = (y^c) / Real.log y := by
          norm_num [ Real.rpow_def_of_pos ( zero_lt_one.trans hy ) ];
          have h_integrable_large_y : ∫ x in Set.Iic c, Real.exp (Real.log y * x) = Real.exp (Real.log y * c) * ∫ x in Set.Iic 0, Real.exp (Real.log y * x) := by
            rw [ ← MeasureTheory.integral_const_mul ];
            rw [ ← MeasureTheory.integral_indicator, ← MeasureTheory.integral_indicator ] <;> norm_num [ Set.indicator ];
            rw [ ← MeasureTheory.integral_add_right_eq_self _ c ] ; congr ; ext x ; split_ifs <;> simp_all +decide [ ← Real.exp_add ] ; ring;
          ...
          rw [ ← this, ← MeasureTheory.integral_Iic_eq_integral_Iio ] ; rw [ ← neg_zero, ← integral_comp_neg_Iic ] ; norm_num;
        rw [ MeasureTheory.integral_div, h_integrable_large_y, div_div, mul_comm ];
      refine' h_integral ▸ MeasureTheory.integral_mono_of_nonneg _ _ _;
      · exact Filter.Eventually.of_forall fun x => norm_nonneg _;
...
lemma integral_y_pow_sub_one_div_z_rect_eq_zero (y : ℝ) (hy : 0 < y) (c : ℝ) (hc : 0 < c) (T : ℝ) (hT : 0 < T) (R : ℝ) (hR : 0 < R) :
    let f := fun z : ℂ => (y^z - 1) / z
    (∫ t in -T..T, f (c + I * t) * I) +
    (∫ x in c..(-R), f (x + I * T)) +
    (∫ t in T..(-T), f (-R + I * t) * I) +
    (∫ x in -R..c, f (x - I * T)) = 0 := by
      -- Define `g(z)` to be `(y^z - 1)/z` for `z ≠ 0` and `log y` for `z = 0`.
      set g : ℂ → ℂ := fun z =>
        if z = 0 then Complex.log y else ((y : ℂ) ^ z - 1) / z;
      -- Show that `g` is holomorphic on $\mathbb{C}$.
      have hg_holomorphic : DifferentiableOn ℂ g Set.univ := by
        ...
        have hg_diff_zero : HasDerivAt g ((Complex.log y)^2 / 2) 0 := by
          ...
          have h_lim : Filter.Tendsto (fun z : ℂ => ((y : ℂ) ^ z - 1 - z * Complex.log y) / z ^ 2) (nhdsWithin 0 {0}ᶜ) (nhds ((Complex.log y) ^ 2 / 2)) := by
            -- We'll use the fact that $e^{z \log y} = \sum_{n=0}^{\infty} \frac{(z \log y)^n}{n!}$ to expand $y^z$.
            have h_expansion : ∀ z : ℂ, (y : ℂ) ^ z = ∑' n : ℕ, (z * Complex.log y) ^ n / Nat.factorial n := by
              intro z; rw [ Complex.cpow_def_of_ne_zero ( Complex.ofReal_ne_zero.mpr hy.ne' ) ] ; rw [ Complex.exp_eq_exp_ℂ ] ; rw [ NormedSpace.exp_eq_tsum_div ] ; ring;
            -- Substitute the expansion into the limit expression.
            suffices h_subst : Filter.Tendsto (fun z : ℂ => (∑' n : ℕ, (z * Complex.log y) ^ (n + 2) / Nat.factorial (n + 2)) / z ^ 2) (𝓝[≠] 0) (nhds ((Complex.log y) ^ 2 / 2)) by
            ...
            suffices h_factor : Filter.Tendsto (fun z : ℂ => (∑' n : ℕ, (Complex.log y) ^ (n + 2) * z ^ n / Nat.factorial (n + 2))) (𝓝[≠] 0) (nhds ((Complex.log y) ^ 2 / 2)) by
              refine' h_factor.congr' _;
              filter_upwards [ self_mem_nhdsWithin ] with z hz using by rw [ eq_div_iff ( pow_ne_zero 2 hz ) ] ; rw [ ← tsum_mul_right ] ; exact tsum_congr fun n => by ring;
            ...
            have h_uniform : ContinuousOn (fun z : ℂ => ∑' n : ℕ, (Complex.log y) ^ (n + 2) * z ^ n / Nat.factorial (n + 2)) (Metric.closedBall 0 1) := by
              refine' continuousOn_tsum _ _ _;
              use fun n => ‖Complex.log y‖ ^ ( n + 2 ) / Nat.factorial ( n + 2 );
        ...
        · refine' ⟨ _, hasDerivWithinAt_iff_tendsto_slope.mpr _ ⟩;
        ...
        · refine' DifferentiableAt.differentiableWithinAt _;
          exact DifferentiableAt.congr_of_eventuallyEq ( show DifferentiableAt ℂ ( fun z => ( ( y : ℂ ) ^ z - 1 ) / z ) z from DifferentiableAt.div ( DifferentiableAt.sub ( DifferentiableAt.cpow ( differentiableAt_const _ ) differentiableAt_id <| by aesop ) <| differentiableAt_const _ ) differentiableAt_id <| by aesop ) <| Filter.eventuallyEq_of_mem ( isOpen_compl_singleton.mem_nhds h ) fun x hx => if_neg hx;
      -- Apply Cauchy's Integral Theorem for rectangles to `g`.
      have h_cauchy_integral : (∫ t in (-T)..T, g (c + Complex.I * t) * Complex.I) + (∫ x in (c)..(-R), g (x + Complex.I * T)) + (∫ t in (T)..(-T), g (-R + Complex.I * t) * Complex.I) + (∫ x in (-R)..c, g (x - Complex.I * T)) = 0 := by
        have h_cauchy_integral : ∀ (a b c d : ℝ), a ≤ b → c ≤ d → (∫ t in (c)..d, g (a + Complex.I * t) * Complex.I) + (∫ x in (a)..b, g (x + Complex.I * d)) + (∫ t in (d)..c, g (b + Complex.I * t) * Complex.I) + (∫ x in (b)..a, g (x + Complex.I * c)) = 0 := by
          ...
          norm_num [ mul_comm Complex.I, ← intervalIntegral.integral_of_le hcd, ← intervalIntegral.integral_of_le hab ];
          exact ⟨ fun h s hs hs' hs'' => by linear_combination -h, fun h => by linear_combination -h ∅ ( by norm_num ) ( by exact ContinuousOn.mono hg_holomorphic.continuousOn fun x hx => by simpa ) ( by intros; exact hg_holomorphic.differentiableAt ( by simpa ) ) ⟩;
        by_cases h : c ≤ -R <;> simp_all +decide [ intervalIntegral ];
        · linarith;
      ...
      refine' congrArg₂ _ ( congrArg₂ _ ( congrArg₂ _ _ _ ) _ ) _ <;> refine' intervalIntegral.integral_congr fun x hx => _ <;> norm_num [ Complex.ext_iff, Complex.exp_re, Complex.exp_im, Complex.log_re, Complex.log_im, Complex.cpow_def ] at *;
      · simp +zetaDelta at *;
...
lemma integral_one_div_z_left_side_value (R T : ℝ) (hR : 0 < R) (hT : 0 < T) :
    ∫ t in T..(-T), 1 / (-R + I * t) * I = 2 * I * Real.arctan (T / R) := by
      ...
      have h_simplify : ∀ t : ℝ, 1 / (-(R : ℂ) + Complex.I * t) * Complex.I = (t - R * Complex.I) / (R^2 + t^2) := by
      ...
      suffices h_split : (∫ t in T..(-T), t / (R^2 + t^2) : ℝ) - Complex.I * (∫ t in T..(-T), R / (R^2 + t^2) : ℝ) = 2 * Complex.I * Real.arctan (T / R) by
        ...
        · norm_num [ div_eq_mul_inv, mul_assoc, mul_comm, mul_left_comm, ← intervalIntegral.integral_ofReal ];
      ...
      have h_integrals : ∀ a b : ℝ, ∫ t in a..b, t / (R^2 + t^2) = (1 / 2) * Real.log (R^2 + b^2) - (1 / 2) * Real.log (R^2 + a^2) ∧ ∫ t in a..b, R / (R^2 + t^2) = Real.arctan (b / R) - Real.arctan (a / R) := by
        intros a b; constructor <;> rw [ intervalIntegral.integral_deriv_eq_sub' ] <;> norm_num [ div_eq_mul_inv ];
        · exact funext fun x => by norm_num [ Real.differentiableAt_log, show R ^ 2 + x ^ 2 ≠ 0 by positivity ] ; ring;
        · exact fun x hx => DifferentiableAt.mul ( differentiableAt_const _ ) ( DifferentiableAt.log ( by norm_num ) ( by positivity ) );
        · exact continuousOn_of_forall_continuousAt fun x hx => ContinuousAt.mul continuousAt_id <| ContinuousAt.inv₀ ( continuousAt_const.add <| continuousAt_id.pow 2 ) <| by positivity;
        · -- By simplifying, we can see that the derivatives are equal.
          ...
          field_simp;
        · exact continuousOn_of_forall_continuousAt fun x hx => ContinuousAt.mul continuousAt_const <| ContinuousAt.inv₀ ( continuousAt_const.add <| continuousAt_id.pow 2 ) <| by positivity;
...
lemma integral_one_div_z_right_side_value (c T : ℝ) (hc : 0 < c) (hT : 0 < T) :
    ∫ t in -T..T, 1 / (c + I * t) * I = 2 * I * Real.arctan (T / c) := by
...
lemma integral_one_div_z_top_value (c T R : ℝ) (hT : 0 < T) :
    ∫ x in c..(-R), 1 / (x + I * T) = log (-R + I * T) - log (c + I * T) := by
      rw [ intervalIntegral.integral_eq_sub_of_hasDerivAt ];
      rotate_right;
...
lemma integral_one_div_z_bottom_value (c T R : ℝ) (hT : 0 < T) :
    ∫ x in -R..c, 1 / (x - I * T) = log (c - I * T) - log (-R - I * T) := by
      rw [ intervalIntegral.integral_eq_sub_of_hasDerivAt ];
      rotate_right;
      use fun x => Complex.log ( x - Complex.I * T );
      · norm_num;
...
/-
The integral of 1/z over the rectangle containing 0 is 2πi.
-/
open Real Complex Set MeasureTheory intervalIntegral Filter Topology
...
lemma integral_one_div_z_rect_eq_two_pi_I (c : ℝ) (hc : 0 < c) (T : ℝ) (hT : 0 < T) (R : ℝ) (hR : 0 < R) :
    ...
    (∫ x in c..(-R), 1 / (x + I * T)) +
    (∫ t in T..(-T), 1 / (-R + I * t) * I) +
    (∫ x in -R..c, 1 / (x - I * T)) = 2 * π * I := by
      -- Use the four helper lemmas to replace the integrals with their values.
      have h_integrals : (∫ t in (-T)..T, 1 / (c + Complex.I * t) * Complex.I) = 2 * Complex.I * Real.arctan (T / c) ∧
                         (∫ x in c..(-R), 1 / (x + Complex.I * T)) = Complex.log (-R + Complex.I * T) - Complex.log (c + Complex.I * T) ∧
                         (∫ t in T..(-T), 1 / (-R + Complex.I * t) * Complex.I) = 2 * Complex.I * Real.arctan (T / R) ∧
                         (∫ x in (-R)..c, 1 / (x - Complex.I * T)) = Complex.log (c - Complex.I * T) - Complex.log (-R - Complex.I * T) := by
                           refine' ⟨ _, _, _, _ ⟩;
      ...
      convert congr_arg₂ ( · + · ) ( congr_arg₂ ( · + · ) ( congr_arg₂ ( · + · ) h_integrals.1 h_integrals.2.1 ) h_integrals.2.2.1 ) h_integrals.2.2.2 using 1 ; ring;
      norm_num [ Complex.ext_iff, Complex.log_re, Complex.log_im ];
...
/-
The integral of y^z/z over the rectangle is 2πi for y > 1.
-/
open Real Complex Set MeasureTheory intervalIntegral Filter Topology
...
lemma integral_boundary_rect_perron_pos (y : ℝ) (hy : 1 < y) (c : ℝ) (hc : 0 < c) (T : ℝ) (hT : 0 < T) (R : ℝ) (hR : 0 < R) :
    ...
    (∫ x in c..(-R), f (x + I * T)) +
    (∫ t in T..(-T), f (-R + I * t) * I) +
    (∫ x in -R..c, f (x - I * T)) = 2 * π * I := by
      convert congr_arg₂ ( · + · ) ( integral_one_div_z_rect_eq_two_pi_I c hc T hT R hR ) ( integral_y_pow_sub_one_div_z_rect_eq_zero y ( by positivity ) c hc T hT R hR ) using 1;
      · norm_num [ sub_div ];
        ...
        any_goals refine' Continuous.inv₀ _ _ ; continuity;
...
lemma integral_norm_bound_small_y_improper (y : ℝ) (hy : 0 < y) (hy1 : y < 1) (c : ℝ) (T : ℝ) (hT : 0 < T) :
    ∫ x in Ioi c, ‖(y : ℂ)^(x + I * T) / (x + I * T)‖ ≤ y^c / (T * |Real.log y|) := by
      -- Use `MeasureTheory.integral_mono_of_nonneg` to bound the integral of the norm by the integral of `fun x => y^x / T`.
      have h_integral_mono : ∫ x in Set.Ioi c, ‖(y : ℂ) ^ (x + Complex.I * T) / (x + Complex.I * T)‖ ≤ ∫ x in Set.Ioi c, y ^ x / T := by
        ...
        · -- The integral of $y^x / T$ over $(c, \infty)$ is convergent.
          have h_integrable : MeasureTheory.IntegrableOn (fun x => y^x) (Set.Ioi c) := by
            ...
            have h_integrable : ∫ x in Set.Ioi c, Real.exp (Real.log y * x) = (1 / (-Real.log y)) * Real.exp (Real.log y * c) := by
      ...
      -- Compute `∫ x in Ioi c, y^x`.
      have h_integral_y_pow : ∫ x in Set.Ioi c, y^x = y^c / (-Real.log y) := by
        simp +decide [ Real.rpow_def_of_pos hy, mul_comm ];
        have := integral_exp_neg_mul_rpow zero_lt_one ( neg_pos.mpr ( Real.log_neg hy hy1 ) );
        convert congr_arg ( fun x : ℝ => Real.exp ( c * Real.log y ) * x ) ( this ) using 1;
        · rw [ ← MeasureTheory.integral_const_mul ] ; rw [ ← MeasureTheory.integral_indicator ( measurableSet_Ioi ), ← MeasureTheory.integral_indicator ( measurableSet_Ioi ) ] ; norm_num [ Set.indicator ] ;
          rw [ ← MeasureTheory.integral_add_right_eq_self _ c ] ; congr ; ext x ; split_ifs <;> simp_all +decide [ ← Real.exp_add ] ; ring;
