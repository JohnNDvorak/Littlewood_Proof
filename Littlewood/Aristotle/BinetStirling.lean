...
noncomputable def binetIntegral (z : ℂ) : ℂ :=
  ∫ t in Set.Ioi 0, binetIntegrand t * Complex.exp (-t * z)
...
/-
The Binet integrand is differentiable on (0, ∞).
-/
open Asymptotics Filter Complex Topology

lemma binet_integrand_differentiable :
    DifferentiableOn ℝ binetIntegrand (Set.Ioi 0) := by
  unfold binetIntegrand
...
lemma bounded_of_continuous_limits {f : ℝ → ℝ}
    ...
    (h_zero : ∃ L, Tendsto f (𝓝[>] 0) (𝓝 L))
    (h_inf : ∃ L, Tendsto f atTop (𝓝 L)) :
    ∃ M, ∀ t ∈ Set.Ioi 0, |f t| ≤ M := by
  obtain ⟨L₀, hL₀⟩ := h_zero
  obtain ⟨Linf, hLinf⟩ := h_inf
  -- Near 0
  obtain ⟨δ, hδ_pos, hδ⟩ : ∃ δ > 0, ∀ t ∈ Set.Ioo 0 δ, |f t| ≤ |L₀| + 1 := by
    have := Metric.tendsto_nhdsWithin_nhds.mp hL₀ 1 one_pos
    obtain ⟨δ, hδ_pos, hδ⟩ := this
    refine ⟨δ, hδ_pos, fun t ht => ?_⟩
    have := hδ ht.1 (by simp [abs_of_pos ht.1]; exact ht.2)
    rw [Real.dist_eq] at this
    have h_abs := abs_sub_abs_le_abs_sub (f t) L₀
    linarith
  -- Near infinity
  obtain ⟨T, hT_pos, hT⟩ : ∃ T > 0, ∀ t ≥ T, |f t| ≤ |Linf| + 1 := by
    ...
    linarith
  -- On compact middle region [δ, T]
  have h_compact : IsCompact (Set.Icc δ T) := isCompact_Icc
  have h_cont_mid : ContinuousOn f (Set.Icc δ T) := h_cont.mono (fun x hx => by
    simp only [Set.mem_Ioi, Set.mem_Icc] at hx ⊢; linarith)
  obtain ⟨M_mid, hM_mid⟩ := h_compact.exists_bound_of_continuousOn h_cont_mid
  -- Combine bounds
  use max (|L₀| + 1) (max (|Linf| + 1) M_mid)
  intro t ht
  simp only [Set.mem_Ioi] at ht
  by_cases h1 : t < δ
  · exact le_trans (hδ t ⟨ht, h1⟩) (le_max_left _ _)
  · by_cases h2 : t ≥ T
    · exact le_trans (hT t h2) (le_max_of_le_right (le_max_left _ _))
    · have : t ∈ Set.Icc δ T := ⟨not_lt.mp h1, le_of_lt (not_le.mp h2)⟩
      exact le_trans (hM_mid t this) (le_max_of_le_right (le_max_right _ _))

/-
The Binet integrand is bounded on (0, ∞).
-/
open Asymptotics Filter Complex Topology

lemma binet_integrand_bounded :
    ∃ M, ∀ t ∈ Set.Ioi 0, |binetIntegrand t| ≤ M := by
  exact bounded_of_continuous_limits
    binet_integrand_continuous
    ⟨1/12, binet_integrand_limit_zero⟩
    ⟨0, binet_integrand_limit_infinity⟩
