...
/-- Monotonicity of the Perron half-budget tower scale. -/
private lemma piHalfBudgetTower_mono :
    Monotone (fun u : ℝ => Real.exp (Real.exp (Real.exp u) / 2)) := by
  intro a b hab
  exact Real.exp_le_exp.mpr <|
    div_le_div_of_nonneg_right
      (Real.exp_le_exp.mpr <| Real.exp_le_exp.mpr hab)
      (by norm_num : (0 : ℝ) ≤ 2)

/-- The Perron half-budget tower scale is unbounded. -/
private lemma piHalfBudgetTower_unbounded :
    ∀ X : ℝ, ∃ B : ℝ, X ≤ Real.exp (Real.exp (Real.exp B) / 2) := by
  intro X
  have h_inner :
      Filter.Tendsto (fun B : ℝ => Real.exp (Real.exp B)) Filter.atTop Filter.atTop :=
    Real.tendsto_exp_atTop.comp Real.tendsto_exp_atTop
  have h_half :
      Filter.Tendsto (fun B : ℝ => (1 / 2 : ℝ) * Real.exp (Real.exp B))
        Filter.atTop Filter.atTop :=
    Filter.Tendsto.const_mul_atTop (by norm_num : (0 : ℝ) < 1 / 2) h_inner
  have h_half' :
      Filter.Tendsto (fun B : ℝ => Real.exp (Real.exp B) / 2)
        Filter.atTop Filter.atTop := by
    simpa [div_eq_mul_inv, one_div, mul_comm, mul_left_comm, mul_assoc] using h_half
  have h_tendsto :
      Filter.Tendsto (fun B : ℝ => Real.exp (Real.exp (Real.exp B) / 2))
        Filter.atTop Filter.atTop :=
    Real.tendsto_exp_atTop.comp h_half'
  have h_event : ∀ᶠ B in Filter.atTop,
      X ≤ Real.exp (Real.exp (Real.exp B) / 2) :=
    (Filter.tendsto_atTop.1 h_tendsto) X
  rcases (Filter.eventually_atTop.1 h_event) with ⟨B, hB⟩
  exact ⟨B, hB B le_rfl⟩

/-- Cofinal lower-bound version of the distinct zero-count tower budget.

Unlike `tower_cap_unbounded_with_eps`, this local form has an explicit lower
height floor and uses the distinct zero count `N`, matching the Perron
canonical residual. -/
theorem zero_count_div_unbounded_with_floor
  ...
  let B0 : ℝ := max B 0
  let H0 : ℝ := max H 4
  let T : ℝ := max (max T0 H0) (Real.exp (6 * Real.pi * B0))
  have hT_ge_T0 : T0 ≤ T := by
    exact le_trans (le_max_left _ _) (le_max_left _ _)
  have hT_ge_H0 : H0 ≤ T := by
    exact le_trans (le_max_right T0 H0) (le_max_left _ _)
  have hT_geH : H ≤ T := (le_max_left H 4).trans hT_ge_H0
  have hT_ge4 : 4 ≤ T := (le_max_right H 4).trans hT_ge_H0
  have hT_ge1 : (1 : ℝ) ≤ T := by linarith [hT_ge4]
  have hT1_pos : 0 < T + 1 := by linarith
  have hB_le_B0 : B ≤ B0 := le_max_left _ _

  have hN_lower : T / (3 * Real.pi) * Real.log T ≤ N T :=
    hT0 T hT_ge_T0
  have hN_div :
  ...
  have hcoef_le :
    ...
    have hleft :
        (1 / (6 * Real.pi)) * Real.log T
          = (1 / 2 : ℝ) * ((1 / (3 * Real.pi)) * Real.log T) := by
      ring
    calc
      (1 / (6 * Real.pi)) * Real.log T
  ...
  exact ⟨T, hT_geH, hT_ge4, hB_le_B0.trans (hB0_le_log.trans hcoef_le)⟩

/-- Cofinal unboundedness of the exact Perron half-budget tower with an
explicit lower height floor. -/
theorem perron_halfBudgetTower_unbounded_with_eps_and_floor
    [ZeroCountingLowerBoundHyp] :
    ∀ X H ε : ℝ, 0 < ε → ε < 1 →
      ∃ T : ℝ,
        H ≤ T ∧
        ...
        X ≤ Real.exp (Real.exp (Real.exp
          (((1 - ε) * ((N T : ℝ) / (T + 1))) / 2)) / 2) := by
  intro X H ε hεpos hεlt
  have hfac_pos : 0 < 1 - ε := by linarith
  rcases piHalfBudgetTower_unbounded X with ⟨B, hXB⟩
  rcases zero_count_div_unbounded_with_floor (2 * B / (1 - ε)) H with
    ⟨T, hHT, hT4, hTbound⟩
  have hhalf :
    ...
    nlinarith
  exact ⟨T, hHT, hT4, hXB.trans (piHalfBudgetTower_mono hhalf)⟩

/-- Pure same-height Perron-threshold growth residual.

This is the actual Perron-side analytic input below the canonical majorant:
after some height depending only on the RH branch, the opaque fixed-height
Perron threshold is below the same half-budget tower at every later height. -/
class PerronThresholdEventuallyBelowExpHalfBudgetHyp
    [PerronSqrtErrorEventuallyAtHeightHyp] : Prop where
  witness :
...
/-- Fixed-`ε = 1/2` chooser-growth residual below the canonical Perron route.

The canonical majorant construction only needs the tower budget at the single
selected tolerance `ε = 1/2`.  This is therefore the narrow Perron-side atom:
eventually, the exact raw threshold chooser is below that same-height
half-budget. -/
def PerronSqrtErrorRawChoiceEventuallyBelowFixedHalfBudgetResidual
    [PerronSqrtErrorEventuallyAtHeightHyp] : Prop :=
  ∀ (_hRH : ZetaZeros.RiemannHypothesis),
    ∃ T0 : ℝ,
      ∀ T : ℝ,
...
/-- Least-valid-threshold residual below the fixed-half comparison.

This is the exact chooser/minimality obstruction: eventually the named
half-budget threshold is a valid Perron threshold, and the raw
`Classical.choose` threshold is no larger than any valid Perron threshold at
the same height.  The second conjunct is not supplied by `Classical.choose_spec`;
that theorem only proves validity of the chosen threshold. -/
def PerronSqrtErrorRawChoiceFixedHalfBudgetLeastValidThresholdResidual
    [PerronSqrtErrorEventuallyAtHeightHyp] : Prop :=
  ∀ (_hRH : ZetaZeros.RiemannHypothesis),
    ∃ T0 : ℝ,
      ∀ T : ℝ,
...
threshold. -/
theorem perronSqrtErrorRawChoiceFixedHalfBudgetThresholdComparison_of_leastValidThresholdResidual
    [PerronSqrtErrorEventuallyAtHeightHyp]
    (h :
      PerronSqrtErrorRawChoiceFixedHalfBudgetLeastValidThresholdResidual) :
    PerronSqrtErrorRawChoiceFixedHalfBudgetThresholdComparisonResidual := by
  intro hRH
  rcases h hRH with ⟨T0, hLeast⟩
  refine ⟨T0, ?_⟩
  intro T hT
  rcases hLeast T hT with ⟨hFixedValid, hLeastAtT⟩
  exact hLeastAtT (perronFixedHalfBudgetThreshold T) hFixedValid
...
theorem perronSqrtErrorRawChoiceFixedHalfBudgetResidual_of_rawChoiceResidual
  ...
  intro hRH
  rcases h hRH with ⟨T0, hBudget⟩
  exact ⟨T0, fun T hT => hBudget T (1 / 2 : ℝ) hT (by norm_num) (by norm_num)⟩
...
theorem perronThresholdTowerExpHalfBudgetCanonicalMajorantResidual_of_eventualThresholdBudget_hyp
  ...
  intro hRH X
  rcases PerronThresholdEventuallyBelowExpHalfBudgetHyp.witness hRH with
    ⟨T0, hPerron⟩
  rcases perron_halfBudgetTower_unbounded_with_eps_and_floor
      (X + 1) T0 (1 / 2 : ℝ) (by norm_num) (by norm_num) with
    ⟨T, hT0, hT4, hX⟩
  have hP :
      perronThreshold hRH T + 1 ≤
        Real.exp (Real.exp (Real.exp
...
/-- The fixed-half raw chooser residual is enough for the canonical Perron
majorant residual, because the canonical construction selects `ε = 1/2`. -/
theorem perronThresholdTowerExpHalfBudgetCanonicalMajorantResidual_of_rawChoiceFixedHalfResidual
    [PerronSqrtErrorEventuallyAtHeightHyp]
    [ZeroCountingLowerBoundHyp]
    (h :
    ...
    PerronThresholdTowerExpHalfBudgetCanonicalMajorantResidual := by
  intro hRH X
  rcases h hRH with ⟨T0, hPerronRaw⟩
  rcases perron_halfBudgetTower_unbounded_with_eps_and_floor
      (X + 1) T0 (1 / 2 : ℝ) (by norm_num) (by norm_num) with
    ⟨T, hT0, hT4, hX⟩
  have hP :
      perronThreshold hRH T + 1 ≤
        Real.exp (Real.exp (Real.exp
          (((1 - (1 / 2 : ℝ)) * ((N T : ℝ) / (T + 1))) / 2)) / 2) := by
    simpa [perronThreshold_eq_rawEventualChoice] using hPerronRaw T hT0
  exact ⟨T, (1 / 2 : ℝ), hT4, by norm_num, by norm_num, max_le hX hP⟩
...
/-- Eventual same-height Perron-threshold budget plus explicit budgeted
finite-zero radii package both Perron-only exact-seed classes.

This endpoint records the reduced Perron atom directly: zero-count cofinality
handles the external lower bound `X`, while
`PerronThresholdEventuallyBelowExpHalfBudgetHyp` is the remaining analytic
same-height control of `perronThreshold`. -/
theorem exactSeedAboveThreshold_perron_of_eventualPerronBudgetAndBudgetedRelativelyDense_hyp
    [PerronSqrtErrorEventuallyAtHeightHyp]
    [ZeroCountingLowerBoundHyp]
    [PerronThresholdEventuallyBelowExpHalfBudgetHyp]
    [TargetAntiFiniteZeroInhomogeneousPhaseBudgetedRelativelyDenseHyp] :
...
/-- Fixed-half raw chooser Perron-threshold growth plus explicit budgeted
finite-zero radii package both Perron-only exact-seed classes.

This is the narrow endpoint for the current `perronThreshold` API: it uses only
the `ε = 1/2` same-height raw chooser budget actually selected by the canonical
majorant construction. -/
theorem exactSeedAboveThreshold_perron_of_rawChoiceFixedHalfPerronBudgetAndBudgetedRelativelyDense_hyp
    [PerronSqrtErrorEventuallyAtHeightHyp]
    [ZeroCountingLowerBoundHyp]
    [TargetAntiFiniteZeroInhomogeneousPhaseBudgetedRelativelyDenseHyp]
    (hPerron :
...
/-- Least-valid-threshold chooser residual plus explicit budgeted finite-zero
radii package both Perron-only exact-seed classes.

This endpoint records the current exact chooser obstruction: the raw eventual
threshold must be least among valid thresholds, and the named half-budget
threshold must itself be valid, at the same height. -/
theorem exactSeedAboveThreshold_perron_of_rawChoiceFixedHalfLeastValidThresholdAndBudgetedRelativelyDense_hyp
    [PerronSqrtErrorEventuallyAtHeightHyp]
    [ZeroCountingLowerBoundHyp]
    [TargetAntiFiniteZeroInhomogeneousPhaseBudgetedRelativelyDenseHyp]
    (hPerron :
