...
/-- Approximate-seed payload at Perron-threshold/tower-cap level.
...
satisfies **approximate** target congruences (within `ε`) for all zeros up
to `T`, and also satisfies the required tower-height cap.

Weakened from the original exact (`= 0`) congruences to approximate
(`‖…‖ ≤ ε`) congruences. Exact congruences are unprovable for N(T) ≥ 2
with Q-independent ordinates. Approximate congruences suffice because
the downstream consumer `TargetTowerArgApproxAbovePerronThreshold`
already uses `‖…‖ ≤ ε`, and 1D Kronecker + Dirichlet approximation
handles the finset simultaneously.
-/
...
/-- Approximate seeds induce the positive arg-above-threshold payload.

The bridge converts `t0`-based congruences to `log(exp t0)`-based ones
(trivial since `log(exp t) = t`), and passes the `‖…‖ ≤ ε` bound through
directly. -/
...
/-- Anti-target approximate seeds induce the anti-target arg-above-threshold
payload. -/
...
/-- Typeclass wrapper for anti-target approximate-seed families. -/
...
/-- 7a/7c endpoint from approximate-seed payload classes. -/
