...
namespace Aristotle.Standalone.HardyApproxFunctionalEqMeanValueLowerDecomp
...
lemma hardyZ_eq_target (t : ℝ) :
    hardyZ t = ‖riemannZeta (1 / 2 + t * Complex.I)‖ := by
  simp [hardyZ, mul_comm]

private def zetaIntegrand (t : ℝ) : ℝ :=
  ‖riemannZeta ((1 / 2 : ℂ) + Complex.I * t)‖ ^ 2
...
private lemma continuous_zetaIntegrand : Continuous zetaIntegrand := by
  ...
  let g : ℝ → ℂ := fun t => (1 / 2 : ℂ) + Complex.I * t
  have hg : Continuous g := by
    simpa [g] using (continuous_const.add (continuous_const.mul Complex.continuous_ofReal))
  ...
  simpa [g] using (hcont_zeta.norm.pow 2)
...
private lemma mean_square_zeta_half_eq_integral (T : ℝ) :
  ...
  simp [mean_square_zeta, zetaIntegrand]

/-- Bridge from `mean_square_zeta (1/2)` to the exact `Ioc 1 T` integral shape. -/
lemma integral_Ioc_one_eq_mean_square_sub (T : ℝ) (hT : 1 ≤ T) :
...
/-- Typeclass-instantiated version, reducing only to `ZetaMeanSquareHalfBound.bound`. -/
...
end Aristotle.Standalone.HardyApproxFunctionalEqMeanValueLowerDecomp
