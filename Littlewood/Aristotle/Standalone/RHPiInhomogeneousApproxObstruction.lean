...
-/
private lemma exists_int_arg_bound_of_target_phase_close
...
theorem antiTargetTowerPhaseCouplingFamily_implies_relation_compatibility_piOverTwo
  ...
  rcases hAntiTargetPhase hRH X with
    ⟨x, hXx, T, hT4, hx1, herr, ε, hεpos, hεlt, hphase, hxUpper⟩
  -- Shift anti-target phases by `π` and reuse the target-phase bound.
  have hTargetLike :
      ∀ ρ ∈ (finite_zeros_le T).toFinset,
        ‖(x : ℂ) ^ (Complex.I * ρ.im) - (-(ρ / ‖ρ‖))‖ ≤ ε := hphase
  refine ⟨x, T, ε, hXx, hT4, hx1, hεpos, hεlt, ?_⟩
  intro m hRel
  let S : Finset ℂ := (finite_zeros_le T).toFinset
  let γ : {ρ // ρ ∈ S} → ℝ := fun i => i.1.im
  ...
  have hApprox :
    ...
    have hρne : i.1 ≠ 0 := by
      intro h0
      have hpow : (x : ℂ) ^ (Complex.I * i.1.im) = 1 := by simp [h0]
      have htarget0 : -(i.1 / ‖i.1‖) = 0 := by simp [h0]
      have hnorm1 : ‖(1 : ℂ) - 0‖ ≤ ε := by
        simpa [hpow, htarget0] using hTargetLike i.1 i.2
      have h1le : (1 : ℝ) ≤ ε := by simpa using hnorm1
      exact (not_le_of_gt hεlt) h1le
    have hx_pos : 0 < x := lt_trans zero_lt_one hx1
    have hcpow :
        (x : ℂ) ^ (Complex.I * i.1.im)
          = Complex.exp ((Real.log x * i.1.im) * Complex.I) := by
      simpa [mul_assoc, mul_comm, mul_left_comm] using
        (cpow_I_im_eq_exp_I_log_mul (x := x) (y := i.1.im) hx_pos)
    have hunit :
        -(i.1 / ‖i.1‖) = Complex.exp ((Complex.arg i.1 + Real.pi) * Complex.I) := by
      exact neg_unit_dir_eq_exp_arg_add_pi hρne
    let θ : ℝ := Real.log x * i.1.im
    let ψ : ℝ := Complex.arg i.1 + Real.pi
    have hcloseExp :
        ‖Complex.exp (θ * Complex.I) - Complex.exp (ψ * Complex.I)‖ ≤ ε := by
      simpa [θ, ψ, S, hcpow, hunit] using hTargetLike i.1 i.2
    have hmul :
        Complex.exp (θ * Complex.I) - Complex.exp (ψ * Complex.I)
          = Complex.exp (ψ * Complex.I) *
              (Complex.exp ((θ - ψ) * Complex.I) - 1) := by
      calc
        Complex.exp (θ * Complex.I) - Complex.exp (ψ * Complex.I)
            = Complex.exp ((ψ + (θ - ψ)) * Complex.I) - Complex.exp (ψ * Complex.I) := by
                congr 1
                ring
        _ = Complex.exp (ψ * Complex.I + (θ - ψ) * Complex.I) - Complex.exp (ψ * Complex.I) := by
              congr 1
              ring
        _ = Complex.exp (ψ * Complex.I) * Complex.exp ((θ - ψ) * Complex.I)
              - Complex.exp (ψ * Complex.I) := by rw [Complex.exp_add]
        _ = Complex.exp (ψ * Complex.I) *
              (Complex.exp ((θ - ψ) * Complex.I) - 1) := by ring
    have hcloseOne :
        ‖Complex.exp (Complex.I * (θ - ψ)) - 1‖ ≤ ε := by
      have hmul_le :
          ‖Complex.exp (ψ * Complex.I) *
              (Complex.exp ((θ - ψ) * Complex.I) - 1)‖ ≤ ε := by
        simpa [hmul] using hcloseExp
      have hcloseOne' :
          ‖Complex.exp ((θ - ψ) * Complex.I) - 1‖ ≤ ε := by
        simpa [norm_mul, Complex.norm_exp_I_mul_ofReal] using hmul_le
    ...
    rcases existsUnique_add_zsmul_mem_Ioc Real.two_pi_pos (θ - ψ) (-Real.pi) with
      ⟨n, hnIoc, -⟩
    let δ : ℝ := (θ - ψ) + (n : ℤ) • (2 * Real.pi)
    have hδIoc : δ ∈ Set.Ioc (-Real.pi) Real.pi := by
      have htmp : δ ∈ Set.Ioc (-Real.pi) (-Real.pi + 2 * Real.pi) := by
        simpa [δ] using hnIoc
      refine ⟨htmp.1, ?_⟩
    ...
    have hsin_lower : (2 / Real.pi) * (|δ| / 2) ≤ Real.sin (|δ| / 2) :=
      Real.mul_le_sin hδ_half_nonneg hδ_half_le
    have hperiod :
        Complex.exp (Complex.I * δ) = Complex.exp (Complex.I * (θ - ψ)) := by
      have hperiod' :
          Complex.exp ((((θ - ψ : ℝ) : ℂ) - ((-n : ℤ) • (2 * (Real.pi : ℂ)))) * Complex.I)
            = Complex.exp (((θ - ψ : ℝ) : ℂ) * Complex.I) := by
        simpa using
          (Complex.exp_mul_I_periodic.sub_zsmul_eq (x := ((θ - ψ : ℂ))) (n := (-n)))
      have hδ_as_sub :
          ((δ : ℝ) : ℂ) = ((θ - ψ : ℝ) : ℂ) - ((-n : ℤ) • (2 * (Real.pi : ℂ))) := by
        simp [δ]
      have hperiod'' :
          Complex.exp (((δ : ℝ) : ℂ) * Complex.I)
            = Complex.exp (((θ - ψ : ℝ) : ℂ) * Complex.I) := by
    ...
    have hδ_rewrite :
        δ =
          (Real.log x * γ i - φ i) - ((-n : ℤ) : ℝ) * (2 * Real.pi) := by
      simp [δ, θ, ψ, γ, φ]
    calc
      ‖Real.log x * γ i - φ i - ((-n : ℤ) : ℝ) * (2 * Real.pi)‖
          = ‖δ‖ := by rw [hδ_rewrite]
      _ = |δ| := by simp [Real.norm_eq_abs]
      _ ≤ (Real.pi / 2) * ε := hδ_bound
  have hRel' : (∑ i, (m i : ℝ) * γ i) = 0 := by
