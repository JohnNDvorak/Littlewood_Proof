...
This packages the sum-level bound from the already proved bridge theorem
`hardyCosIntegralAlternatingSqrtDecomposition_of_sqrtMode`, under the single
mode-wise hypothesis `HardyCosIntegralSqrtModeBoundHyp`. -/
theorem hardy_cos_integral_weighted_sum_bound
    ...
    ∃ C > 0, ∀ T : ℝ, T ≥ 2 →
      |∑ n ∈ Finset.range (hardyN T),
        ...
        ≤ C * ((hardyN T : ℝ) + 1) := by
  exact
    (HardyFirstMomentWiring.hardyCosIntegralAlternatingSqrtDecomposition_of_sqrtMode
      HardyFirstMomentWiring.HardyCosIntegralSqrtModeBoundHyp.bound).bound
