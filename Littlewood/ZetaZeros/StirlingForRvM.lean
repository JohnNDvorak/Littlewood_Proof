...
/-! ## Part 5: Contour Integral Evaluation

The deep analytic content: evaluating the contour integral of (ξ'/ξ)
over the rectangle boundary. This is where the argument principle
meets the Stirling and zeta bounds.

The contour integral decomposes into:
  (1/2πi) ∮_∂R (ξ'/ξ)(s) ds
  = Stirling_contribution + S(T)_contribution + polynomial_contribution + O(1)

The Stirling contribution gives the main term of RvM. -/

/-- The contour integral evaluation for the RvM formula.

    This states that there exist constants C, T₀ such that for T ≥ T₀:
    N(T) = (T/2π)log(T/2π) - T/(2π) + bounded_error

    where the bounded error satisfies |error| ≤ C · log T.

    This is the composition of:
    - Argument principle: N(T) = logIntegralRect for ξ
    - Decomposition: ξ'/ξ = Γ'/Γ contribution + ζ'/ζ contribution + polynomial
    - Stirling for Γ'/Γ integral: gives main term + O(1)
    - Backlund for ζ'/ζ integral: S(T) = O(log T) (proved above)
    - Standard bounds on horizontal edges and far vertical edges: O(1)

    SORRY STATUS: This packages the contour evaluation. The argument
    principle is proved (`argument_principle_rect_entire`). What remains:
    (i) Decomposition of logDeriv of ξ into Γ and ζ pieces
    (ii) Integration of the digamma along vertical lines (Stirling)
    (iii) Bounding the horizontal edge integrals -/
