...
theorem distToInt_add_le (x y : ℝ) : ‖x + y‖ᵢₙₜ ≤ ‖x‖ᵢₙₜ + ‖y‖ᵢₙₜ := by
  unfold distToInt
  calc |x + y - round (x + y)|
      ≤ |x + y - (round x + round y)| := round_le _ (round x + round y)
    _ = |(x - round x) + (y - round y)| := by ring_nf
    _ ≤ |x - round x| + |y - round y| := abs_add _ _
...
theorem distToInt_nsmul_le (x : ℝ) (n : ℕ) : ‖(n : ℝ) * x‖ᵢₙₜ ≤ n * ‖x‖ᵢₙₜ := by
  ...
  | succ k ih =>
    ...
    calc ‖(↑(k + 1) : ℝ) * x‖ᵢₙₜ
        ≤ ‖(↑k : ℝ) * x‖ᵢₙₜ + ‖x‖ᵢₙₜ := by rw [h1]; exact distToInt_add_le _ _
      _ ≤ ↑k * ‖x‖ᵢₙₜ + ‖x‖ᵢₙₜ := by linarith
      _ = ↑(k + 1) * ‖x‖ᵢₙₜ := by push_cast; ring

/-- Inhomogeneous 1D Dirichlet approximation (≤ bound).
    For any θ, α ∈ ℝ and N ≥ 1, there exists n with 0 ≤ n ≤ N such that
    ‖n * θ - α‖ᵢₙₜ ≤ 1/N.

    Proof: The N+1 points {frac(j*θ - α) : j=0,...,N} in [0,1) are mapped
    to N intervals [k/N, (k+1)/N). For N = 1: trivial. For N ≥ 2:
    - If any point lands in [0, 1/N): done via frac branch of distToInt.
    - If any point lands in [(N-1)/N, 1): done via 1-frac branch.
    - Otherwise N+1 points in N-2 intervals: contradiction by pigeonhole. -/
theorem inhomogeneous_dirichlet_1d (θ α : ℝ) (N : ℕ) (hN : 0 < N) :
    ∃ n : ℕ, n ≤ N ∧ ‖(n : ℝ) * θ - α‖ᵢₙₜ ≤ 1 / N := by
  classical
  -- N = 1: distToInt ≤ 1/2 ≤ 1/1
  by_cases hN1 : N = 1
  · subst hN1
    refine ⟨0, by omega, ?_⟩
    simp only [Nat.cast_zero, zero_mul, zero_sub]
    calc ‖-α‖ᵢₙₜ = ‖α‖ᵢₙₜ := distToInt_neg α
      _ ≤ 1 / 2 := distToInt_le_half α
      _ ≤ 1 / 1 := by norm_num
  -- N ≥ 2
  have hN2 : 2 ≤ N := by omega
  have hNpos : (0 : ℝ) < N := by exact_mod_cast hN
  let f : Fin (N + 1) → ℕ := fun j => ⌊fract ((j : ℝ) * θ - α) * N⌋₊
  have hf_lt : ∀ j, f j < N := by
    intro j
    have hnn : 0 ≤ fract ((j : ℝ) * θ - α) * N :=
      mul_nonneg (fract_nonneg _) (by exact_mod_cast Nat.zero_le N)
    exact (Nat.floor_lt hnn).2 (by
      exact_mod_cast (show fract _ * (N : ℝ) < N from by nlinarith [fract_lt_one ((j : ℝ) * θ - α)]))
  -- Case 1: some j has f j = 0 (frac < 1/N)
  by_cases h0 : ∃ j : Fin (N + 1), f j = 0
  · obtain ⟨⟨j, hj⟩, hfj⟩ := h0
    refine ⟨j, by omega, ?_⟩
    have hnn : 0 ≤ fract ((j : ℝ) * θ - α) * N :=
      mul_nonneg (fract_nonneg _) (by exact_mod_cast Nat.zero_le N)
    have hfl : fract ((j : ℝ) * θ - α) < 1 / N := by
      rw [lt_div_iff₀ hNpos, mul_comm]
      by_contra hge; push_neg at hge
      have : 1 ≤ ⌊fract ((j : ℝ) * θ - α) * N⌋₊ := by
        rwa [Nat.one_le_iff_ne_zero, Nat.floor_ne_zero_iff hnn]
      simp only [f] at hfj; omega
    calc ‖(j : ℝ) * θ - α‖ᵢₙₜ
        ≤ fract ((j : ℝ) * θ - α) := by
          unfold distToInt; rw [abs_sub_round_eq_min]; exact min_le_left _ _
      _ ≤ 1 / N := hfl.le
  -- Case 2: some j has f j = N - 1 (frac ≥ (N-1)/N, so 1-frac ≤ 1/N)
  · by_cases hNm1 : ∃ j : Fin (N + 1), f j = N - 1
    · obtain ⟨⟨j, hj⟩, hfj⟩ := hNm1
      refine ⟨j, by omega, ?_⟩
      have hnn : 0 ≤ fract ((j : ℝ) * θ - α) * N :=
        mul_nonneg (fract_nonneg _) (by exact_mod_cast Nat.zero_le N)
      have hge : (N - 1 : ℝ) ≤ fract ((j : ℝ) * θ - α) * N := by
        have := Nat.floor_le hnn
        simp only [f] at hfj
        exact_mod_cast (hfj ▸ this)
      calc ‖(j : ℝ) * θ - α‖ᵢₙₜ
          ≤ 1 - fract ((j : ℝ) * θ - α) := by
            unfold distToInt; rw [abs_sub_round_eq_min]; exact min_le_right _ _
        _ ≤ 1 / N := by nlinarith [div_le_iff₀ hNpos]
    -- Case 3: no j in bucket 0 or N-1 → contradiction
    · push_neg at h0 hNm1
      exfalso
      -- N = 2: buckets are {0,1}. Both excluded → impossible since f j < 2.
      by_cases hN3 : N ≤ 2
      · have hN2' : N = 2 := by omega
        have h1 := hf_lt ⟨0, by omega⟩
        have h2 := h0 ⟨0, by omega⟩
        have h3 := hNm1 ⟨0, by omega⟩
        subst hN2'; omega
      -- N ≥ 3: N+1 points into {1,...,N-2} (N-2 buckets), N+1 > N-2
      push_neg at hN3
      let g : Fin (N + 1) → Fin (N - 2) := fun j =>
        ⟨f j - 1, by
          have h1 : 1 ≤ f j := Nat.one_le_iff_ne_zero.mpr (h0 j)
          have h2 : f j ≤ N - 2 := by have := hf_lt j; have := hNm1 j; omega
          omega⟩
      have hcard : Fintype.card (Fin (N - 2)) < Fintype.card (Fin (N + 1)) := by
        simp; omega
      obtain ⟨i, j, hij, _⟩ := Fintype.exists_ne_map_eq_of_card_lt g hcard
      exact hij rfl
