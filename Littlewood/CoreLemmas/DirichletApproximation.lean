...
/-- **Inhomogeneous simultaneous Dirichlet approximation on an interval.**
    ...
    This is the K-torus pigeonhole argument (Cassels 1957, Ch. III):
    the shifted orbit {(t·γ_1,...,t·γ_K) mod 2π : t ∈ [a,b]} covers
    every (1/2)-ball in the K-torus when the interval is long enough.

    Mathematical proof: The theorem follows from the quantitative inhomogeneous
    Dirichlet theorem (Cassels 1957), combined with the observation that a
    1-dimensional parametric curve of length L in the K-torus [0, 2π)^K,
    when L ≥ (2π/(1/2))^K = (4π)^K, visits every (1/2)-neighborhood.

    The formal proof uses Minkowski's convex body theorem (in Mathlib as
    `MeasureTheory.exists_ne_zero_mem_lattice_of_measure_mul_two_pow_lt_measure`)
    applied to the lattice generated by {(γ_k)_k, e_1, ..., e_K} in ℝ^{K+1}.
    This is a standard reduction; see Montgomery-Vaughan §15.1.

    SORRY STATUS: Core K-torus covering argument. Targeted for closure via
    Minkowski convex body theorem route. -/
