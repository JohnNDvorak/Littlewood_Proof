...
### 2026-04-29 Round 50 - Close Fixed-Window Membership Stability

- Classification: `PROOF_CLOSED`.
- Exact theorem attacked:
  endpoint-exclusion/eventual moving-window membership stability
  `∀ p ∈ slab, ∀ᵐ t ∂volume.restrict (Set.Ioc (-16) 16),
      ∀ᶠ q in 𝓝[slab] p,
        (t ∈ Set.Ioc (-q.2) q.2 ↔ t ∈ Set.Ioc (-p.2) p.2)`.
- Code facts banked:
  added
  `small_T_perronVerticalFixedWindow_membership_eventually_of_ne_endpoints`,
  proving local stability for a fixed height away from the two moving
  endpoints `t = p.2` and `t = -p.2`.  Added
  `small_T_perronVerticalFixedWindow_membership_eventually_ae_slab16`,
  discharging the DCT membership atom by excluding the two endpoint
  singletons, which are null for `volume` and hence for the restricted fixed
  window measure.
- Shape check:
  this is an endpoint-a.e. statement only.  It does not assert joint
  continuity of the moving indicator and does not return to unconditional
  closed-cutoff zero-set constancy.
- Failed/demoted routes:
  did not try to prove fixed-window dominated convergence outright in this
  pass.  The remaining DCT inputs are now the unwindowed integrand
  convergence/measurability and the local integrable-majorant atom.
- Circular/forbidden routes avoided:
  no use of shifted remainder atoms, public main imports,
  `general_formula_accessible`, `ContourRemainderBoundHyp.bound`,
  `SmallTPerronBoundHyp`, or `perron_tail_bound_core`.
...
- Validation:
  ...
  guard.  Both passed.
- Smallest next theorem:
  prove the fixed-window unwindowed-integrand convergence atom for
...
### 2026-04-29 Round 57 - Close Transition Slab Compactness Bridge

- Classification: `PROOF_REDUCTION`.
- Exact theorem attacked:
  the finite transition bounded-image atom over
  `16 <= x <= Xtail`, `2 <= T <= 16`, feeding the cutoff-`16`
  normalized tail split.
- Code facts banked:
  added `small_T_concrete_contour_remainder_transition_bddAbove_image_from_continuousOn`,
  proving the transition bounded-image atom from `ContinuousOn` of
  `perronVerticalContourRemainderNormalized` on the closed transition
  rectangle.  Added
  ...
  then wired the result through
  `small_T_concrete_contour_remainder_normalized_sup_from_finiteZeros_transition_continuousOn_and_asymptotic_tail`
  and
  `small_T_linear_window_bound_hyp_from_concrete_contour_remainder_finiteZeros_transition_continuousOn_and_asymptotic_tail`.
- Shape check:
  the compactness argument is only for the bounded transition rectangle.
  The unbounded `x >= Xtail` side remains a separate eventual asymptotic tail
  hypothesis, and the explicit `16 <= Xtail` hypothesis is carried on the
  transition/tail handoff surface.
- Failed/demoted routes:
  did not attempt compactness on the unbounded tail and did not assert the
  legacy pure-log small-`T` absorption.
- Circular/forbidden routes avoided:
  no use of shifted remainder atoms, public main imports,
  `general_formula_accessible`, `ContourRemainderBoundHyp.bound`,
  `SmallTPerronBoundHyp`, broad providers, dummy witnesses, new axioms, or
...
- Validation:
  ...
  guard.
- Smallest next theorem:
  choose an explicit `Xtail` such as `32`, then prove either transition
  continuity of
  `fun p => perronVerticalContourRemainderNormalized p.1 p.2` on
...
### 2026-04-29 Round 60 - Close Transition Fixed-Window Majorant

- Classification: `PROOF_CLOSED`.
- Exact theorem attacked:
  transition local integrable-majorant input for
  `perronVerticalFixedWindowIntegrandParam` on
  `16 <= x <= Xtail`, `2 <= T <= 16`, with measure restricted to
  `Set.Ioc (-16) 16`.
- Code facts banked:
  added `small_T_perronVerticalIntegrand_continuousOn_x_height_transition_box`,
  proving joint continuity of the unwindowed Perron integrand on the finite
  transition `x`/height box
  `16 <= x <= Xtail`, `-16 <= t <= 16`.  Added
  `small_T_perronVerticalIntegrand_bddAbove_norm_image_transition_box`,
  deriving bounded image from compactness of that rectangle.  Added
  `small_T_perronVerticalFixedWindowIntegrand_integrable_majorant_transition_of_bddAbove`
  and the closed input
  `small_T_perronVerticalFixedWindowIntegrand_integrable_majorant_transition`,
  feeding the transition fixed-window DCT handoff.
- Shape check:
  compactness is only used on the finite transition `x`/height box; the
  unbounded `x >= Xtail` asymptotic tail remains separate.  The theorem does
  not touch zero-sum continuity or closed zero-set local constancy.
- Failed/demoted routes:
  did not attempt all DCT inputs or the full public small-`T` route.  The
  majorant route was scale-safe because it is only a local boundedness input
  for a finite transition rectangle.
- Circular/forbidden routes avoided:
  no use of shifted remainder atoms, public main imports,
  `general_formula_accessible`, `ContourRemainderBoundHyp.bound`,
  `SmallTPerronBoundHyp`, broad providers, dummy witnesses, new axioms, or
...
- Validation:
  ...
  guard.
- Smallest next theorem:
  prove another transition fixed-window DCT input, preferably
  `AEStronglyMeasurable` for
  `perronVerticalFixedWindowIntegrandParam` on the transition rectangle by
  mirroring the slab16 measurability lemmas, or prove the transition a.e.
