...
instance : ExplicitFormulaPsiSmoothedHyp where
  formula := by
    intro k x hx
    sorry

instance : ExplicitFormulaIntegralHyp where
  formula := by
    intro x hx
    sorry

instance : ExplicitFormulaDoubleIntegralHyp where
  formula := by
    intro x hx
    sorry

instance : PsiMellinHyp where
  representation := by
    intro x hx c hc
    sorry

instance : MellinContourShiftHyp where
  shift := by
    intro x hx c hc
    sorry

instance : ZeroSumBoundRHHyp where
  bound := by
    intro hRH x hx
    sorry

instance : PsiErrorBoundHyp where
  bound := by
    intro x hx
    sorry

instance : PsiErrorBoundRHHyp where
  bound := by
    intro hRH x hx
    sorry
