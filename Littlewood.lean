...
import Littlewood.Aristotle.ZetaRealNonvanishing             -- ζ(x) ≠ 0 for real x > 0, ζ'/ζ analytic on reals (0 sorries) ✓
import Littlewood.Aristotle.PringsheimTheorem                -- Pringsheim's theorem: non-neg coeffs → singularity at radius (0 sorries) ✓
